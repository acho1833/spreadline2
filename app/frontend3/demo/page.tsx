...
/**
 * SpreadLine Frontend3 Demo Page
 *
 * Fetches data from /api/nodeFetchSpreadLine4 endpoint
 */
...
function SpreadLineDemo() {
  ...
  const { data, loading, error, setData } = useSpreadLineData('/api/nodeFetchSpreadLine4');
  ...
  useEffect(() => {
    fetch('/api/nodeFetchSpreadLine4')
      .then((res) => res.text())
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-gray-700 text-xl">Loading SpreadLine data from API...</div>
        </div>
      </div>
    );
  }
          ...
          <div className="px-4 py-3 flex items-center justify-between">
            <div>
              <h1 className="text-xl font-black text-gray-900">
                SpreadLine <span className="text-orange-600">Frontend3 Demo</span>
              </h1>
              <p className="text-xs text-gray-500">
                {data.storylines.length} entities | {data.blocks.length} blocks | Ego: {data.ego}
              </p>
              <p className="text-xs text-blue-500">
                API: /api/nodeFetchSpreadLine4
              </p>
            </div>
