...
export class SpreadLinesVisualizer {
  ...
  visualize(container: SVGSVGElement | string): void {
    ...
    const toShowLabel = Math.min(d3.quantile(lifeSpans, 0.8) || 20, 20);
    ...
    this.chartContainer
      ...
      .join(enter => {
        const container = enter
          ...
          .on('click', function (event, d) {
            self._blockUpdate(event, d as Block);
          });
  ...
  private _blockUpdate = (event: MouseEvent, d: Block): void => {
    ...
    const supplement = {
      ...
      reference: this.data.reference?.filter(e => e.year === d.time) || [],
    };
  ...
  private _activateBrush(timeLabels: TimeLabel[], blocks: Block[]): void {
    ...
    (d3.select('#time-container') as any).call(brush).on('dblclick', dblclicked);
  }
  ...
  applyFilter(yearsFilter: number = 1, crossingOnly: boolean = false): void {
    ...
    // Get names of entities that are NOT crossing (they are members of some block)
    const nonCrossing = new Set<string>();
    if (crossingOnly) {
      blocks.forEach(block => {
        block.names.forEach(name => {
          if (name !== ego) {
            nonCrossing.add(name);
          }
        });
      });
    }
    ...
    // If crossingOnly is checked, also hide non-crossing names
    if (crossingOnly) {
      storylines.forEach(s => {
        if (s.name !== ego && nonCrossing.has(s.name)) {
          toHide.add(s.name);
        }
      });
    }
