...
function computeBlock(
  ...
  if (hops[0].length === 0) {
    ...
    const topHopExtents = getExtents(topHops, p => p.posY);
    if (topHopExtents.length === 2) {
  ...
  if (hops[4].length === 0) {
    ...
    const bottomHopExtents = getExtents(bottomHops, p => p.posY);
    if (bottomHopExtents.length === 2) {
