...
export class SpreadLinesVisualizer {
  ...
  visualize(container: SVGSVGElement | string): void {
    ...
    // Set dimensions BEFORE getting bbox for brush
    chartContainer
    ...
    // Ensure valid brush extent with minimum width
    const brushWidth = Math.max(bbox.width - this.margin.right, 100);
    this.brushComponent.brush = d3.brushX().extent([
      [0, startY],
      [brushWidth, startY + 30],
    ]);
