...
function SpreadLineDemo() {
  ...
  // Handle data editor changes - increment version to force re-render
  ...
  // Generate fill lines for storylines passing through expanded blocks
  const generateFillLines = useCallback((storyline: Storyline): string[] => {
    ...
    data.blocks.forEach((block) => {
      ...
      // Check if this storyline passes through this block (not a member of the block)
      ...
      // Look through the storyline's path segments to find one that crosses this posX
      for (let i = 0; i < storyline.lines.length; i++) {
        ...
        if (startX <= tl.posX && endX >= tl.posX) {
          ...
          // Calculate the shift at this position
          const baseShift = getShiftX(tl.posX);

          // Create a horizontal fill line through the expanded block
          const fillStartX = tl.posX - data.blockWidth / 2 + baseShift - expandW / 2;
          const fillEndX = tl.posX + data.blockWidth / 2 + baseShift + expandW / 2;
          ...
          <div className="px-4 py-3 flex items-center justify-between">
            <div>
              <h1 className="text-xl font-black text-white">
                SpreadLine <span className="text-cyan-400">Demo v3</span>
              </h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <a href="/react-design3" className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm">
                Back to Docs
              </a>
            </div>
          </div>
