...
export default function DemoPage() {
  ...
  // Calculate shift for expanded blocks
  ...
  // Create pill-shaped path for block background
  const createPillPath = (block: Block, tl: TimeLabel, expandW: number, baseShift: number, data: SpreadLineData): string => {
    ...
    const leftX = tl.posX - data.blockWidth / 2 + baseShift - expandW / 2;
    const rightX = tl.posX + data.blockWidth / 2 + baseShift + expandW / 2;

    // Create pill shape: start top-left, go right, curve down, go left, curve up
    return `
      M ${leftX + radius} ${minY}
          ...
          <div className="px-4 py-3 flex items-center justify-between">
            <div>
              <h1 className="text-xl font-black text-white">
                SpreadLine <span className="text-cyan-400">Demo v2</span>
              </h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <a href="/react-design2" className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm">
                Back to Docs
              </a>
            </div>
          </div>
              ...
              {data.timeLabels.map((tl) => {
                ...
                return (
                  <g key={tl.label} transform={`translate(${shift}, 0)`}>
                    <text x={tl.posX} y={-40} fill="#94a3b8" fontSize="14" textAnchor="middle" fontWeight="bold">
                      {tl.label}
                    </text>
                    <line
                      x1={tl.posX} y1={-25}
                ...
                {data.storylines.map((sl) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Labels (rendered here for storylines) */}
                      ...
                      {sl.marks.map((mark, idx) => {
                        ...
                        // Offset markers to be outside block bounds
                ...
                {data.blocks.map((block) => {
                  ...
                  return (
                    ...
                    >
                      {/* Clickable area */}
                      <rect
                        x={tl.posX - data.blockWidth / 2 + baseShift - expandW / 2 - 10}
                        y={minY - 10}
                        width={data.blockWidth + expandW + 20}
                        height={maxY - minY + 20}
                        fill="transparent"
                      ...
                      {/* Pill-shaped background - ALWAYS visible */}
                      <path
                        d={createPillPath(block, tl, expandW, baseShift, data)}
                        fill="#1e293b"
