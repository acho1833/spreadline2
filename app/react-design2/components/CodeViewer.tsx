...
// Simple token-based syntax highlighting that doesn't break
function highlightSyntax(code: string, language: string): React.ReactNode[] {
  ...
  return lines.map((line, lineIdx) => {
    ...
    while (remaining.length > 0) {
      ...
      // Keywords
      const keywordMatch = remaining.match(/^(const|let|var|function|return|if|else|for|while|import|export|default|interface|type|extends|class|new|async|await|from)\b/);
      if (keywordMatch) {
      ...
      // Types
      const typeMatch = remaining.match(/^(string|number|boolean|void|null|undefined|any|React|Set|Map)\b/);
      if (typeMatch) {
