...
// Custom hook to keep callback refs up-to-date without triggering re-renders
...
export default function SpreadLineChart({
  ...
  const initVisualization = useCallback(() => {
    ...
    // Note: onBlockExpandRef and onFilterChangeRef are stable refs, not included in deps
  ...
  /**
   * Handle filter changes without full reinitialization
   */
  useEffect(() => {
    if (visualizerRef.current) {
